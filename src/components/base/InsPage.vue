<template>
  <div class="page_warrper" :style="styla">
    <span class="last" :class="{ 'noClick': Page === 1 }" @click="current--">{{$t('pager.Last')}} </span>
    <select v-model="current" class="selectPage">
      <option v-for="(item,index) in T" :value ="index + 1" :key="index" >{{ $t('pager.per') + ' ' + (index + 1) + ' ' + $t('pager.page')}}</option>
    </select>
    <span class="pageing"> / {{$t('pager.Total') + Math.ceil(this.total / this.pageNum) + $t('pager.page')}} </span>
    <span class="next" :class="{ 'noClick': Page === T.length }" @click="current++">{{$t('pager.Next')}} </span>
  </div>
</template>
<script lang="ts">
